...
import com.grpc.server.utils.DiskFileStorage;
		...
		return new StreamObserver<>() {
			...
			public void onNext(FileUploadRequest fileUploadRequest) {
				...
				try {
					...
					responseObserver.onError(io.grpc.Status.INTERNAL
							.withDescription("cannot write data due to : " + e.getMessage()).asRuntimeException());
				}
			}
			...
			public void onCompleted() {
				...
				try {
					...
					if (totalBytesReceived == fileMetadata.getContentLength()) {
						...
						responseObserver
								.onError(io.grpc.Status.FAILED_PRECONDITION
										.withDescription(String.format("expected %d but received %d",
					...
					responseObserver.onError(io.grpc.Status.INTERNAL
							.withDescription("cannot save data due to : " + e.getMessage()).asRuntimeException());
					return;
				}

				responseObserver.onNext(FileUploadResponse.newBuilder().setFileName(fileMetadata.getFileNameWithType())
						.setUploadStatus(UploadStatus.SUCCESS).build());
