...
public class ShardingApp {
	...
	public static void main(String[] args) throws Exception {
		...
		var todosToInsert = Stream.generate(ShardingApp::randomTodo).limit(todos).toList();
		 ...
		 * System.out.println("Data created!"); } else {
		 * System.out.println("Didn't finish creating data in 30 seconds..."); }
		 */

		 System.out.println(repository.ofId(185918));
		 System.out.println(repository.ofId(578987));
		 System.out.println(repository.ofId(783405));

		 System.out.println(repository.allOfNameLike("az").size());
	}

	private static List<JdbcTemplate> shardsJdbcTemplates(int shards) {
		return IntStream.range(0, shards).mapToObj(i -> {
			try {
				var connection = DriverManager.getConnection("jdbc:postgresql://localhost:555%d/postgres".formatted(i),
						"postgres", "postgres");
